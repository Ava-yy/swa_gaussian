...
for i in range(args.num_samples):
    ...
    VGG16_model = torch.load("../../food-101/food101_vgg16_finetune.pt")
    vgg16_params_name = VGG16_model.state_dict().keys()
    vgg16_state_dict = {}

    # print('swag_model params : ',swag_model.params)
    # print('swag_model state dict : ',swag_model.state_dict())
    ...
    for param_name in resnet50_params_name:
        ...
        if param_name.split('.')[-1]=='weight':
            vgg16_state_dict[param_name] = swag_model.params[idx][0].weight
            idx +=1
        elif param_name.split('.')[-1] =='bias':
            vgg16_state_dict[param_name] = swag_model.params[idx][0].bias
            idx +=1
        else:
            vgg16_state_dict[param_name] = swag_model.state_dict()['base.'+param_name]

    print('resnet_state_dict : ',resnet50_state_dict.keys())
    utils.save_model_sample_checkpoint(
        args.swag_sample_model_dir, i, name="swag_sample", state_dict=vgg16_state_dict
    )
    checkpoint = torch.load(os.path.join(args.swag_sample_model_dir,'swag_sample-'+str(i)+'.pt'))
